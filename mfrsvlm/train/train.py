...
from mfrsvlm import conversation as conversation_lib
from mfrsvlm.constants import (DEFAULT_IM_END_TOKEN, DEFAULT_IM_START_TOKEN,
                               DEFAULT_IMAGE_TOKEN, IGNORE_INDEX)
from mfrsvlm.mm_utils import tokenizer_image_token
from mfrsvlm.model import *
from mfrsvlm.train.vhm_trainer import VHMTrainer
...
class ModelArguments:
    ...
    deepstack_detail_layers: Optional[List[float]] = field(
        default=None,
        metadata={"help": "DeepStack: target ViT layer ratios or indices."},
    )
    deepstack_window_scales: Optional[List[float]] = field(
        default=None,
        metadata={"help": "DeepStack: multi-scale window ratios."},
    )
    deepstack_downsample_factor: Optional[int] = field(
        default=None,
        metadata={"help": "DeepStack: token downsample factor when stitching."},
    )
...
def train():
    ...
    def _param_numel(parameter):
        if hasattr(parameter, "ds_numel"):
            return parameter.ds_numel
        return parameter.numel()

    local_total_params = sum(_param_numel(p) for p in model.parameters())
    local_trainable_params = sum(
        _param_numel(p) for p in model.parameters() if p.requires_grad
    )

    has_ds_numel = any(hasattr(p, "ds_numel") for p in model.parameters())

    if dist.is_available() and dist.is_initialized() and not has_ds_numel:
        device = torch.device("cuda", torch.cuda.current_device()) if torch.cuda.is_available() else torch.device("cpu")
        total_tensor = torch.tensor([local_total_params], device=device, dtype=torch.long)
        trainable_tensor = torch.tensor([local_trainable_params], device=device, dtype=torch.long)
        dist.all_reduce(total_tensor, op=dist.ReduceOp.SUM)
        dist.all_reduce(trainable_tensor, op=dist.ReduceOp.SUM)
        global_total_params = int(total_tensor.item())
        global_trainable_params = int(trainable_tensor.item())
    else:
        global_total_params = local_total_params
        global_trainable_params = local_trainable_params

    frozen_params = global_total_params - global_trainable_params
    trainable_ratio = (global_trainable_params / global_total_params) if global_total_params else 0.0
    rank0_print(
        f"Parameter breakdown -> total: {global_total_params:,}, "
        f"trainable: {global_trainable_params:,} ({trainable_ratio:.2%}), "
